...
function PreProductionForm({ job, existing }) {
  ...
  const [currentPP, setCurrentPP] = useState(existing || null);
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      existing_layers: roof.layers||null, osb_sheets: roof.osb ? parseInt(roof.osb) : null,
      ...
      quarrix_plugs: parseInt(roof.quarrixPlugs)||0, box_vent_count: parseInt(roof.boxVentCount)||0, box_vent_color: roof.boxVentColor||null,
      material_spot: roof.materialSpot||null, dump_trailer: roof.dumpTrailer||null, roof_notes: roof.notes||null,
      gutter_color: gutters.color||null, gutter_size: gutters.size||null, gutter_guards: gutters.guards === 'Yes', guard_type: gutters.guardType||null, gutter_notes: gutters.notes||null,
      siding_brand: siding.brand||null, siding_type_line: siding.typeLine||null, siding_style: siding.style||null, siding_color: siding.color||null, siding_notes: siding.notes||null,
      sf_material: sf.material||null, sf_color: sf.color||null, soffit_width: sf.soffitWidth||null, soffit_vented: sf.vented === 'Yes', fascia_height: sf.fasciaHeight||null, sf_notes: sf.notes||null,
    ...
    <div style={{ minHeight: '100vh', background: C.panel, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: font }}>
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: 40, maxWidth: 480, width: '100%', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setSubmitted(false)} style={{ background: C.white, border: `1px solid ${C.border}`, color: C.dark, padding: '11px 24px', borderRadius: 4, cursor: 'pointer', fontSize: 13, fontFamily: font }}>
            Edit Form
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 740, margin: '0 auto', padding: '22px 16px 60px' }}>
        ...
        {!noScope && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 10 }}>
            ...
            <button onClick={handleSubmit} disabled={saving} style={{ background: C.gold, color: C.black, border: 'none', padding: '13px 40px', borderRadius: 4, fontSize: 14, fontWeight: 700, cursor: saving ? 'wait' : 'pointer', fontFamily: font, boxShadow: '0 2px 6px rgba(245,184,0,0.35)', opacity: saving ? 0.7 : 1 }}>
